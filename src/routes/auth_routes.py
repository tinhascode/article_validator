...
class AuthRoutes:
    ...
    async def login(
        ...
        cookie_name = jwt_config.refresh_cookie_name or "refresh_token"
        ...
        if existing_raw:
            existing_rt = token_service.lookup_by_raw(existing_raw)
            if existing_rt and existing_rt.is_active():
                raw_refresh = existing_raw
            else:
                raw_refresh = token_service.create_refresh_token(
                    user_id=str(user.id),
        ...
        response.set_cookie(
            ...
            max_age=max_age,
            path="/",
            domain=jwt_config.refresh_cookie_domain or None,
        )

        csrf_cookie_name = getattr(jwt_config, "refresh_csrf_cookie_name", None) or "refresh_csrf"
        existing_csrf = None
        if request:
    ...
    async def refresh(
        ...
        user = auth_service.db.get(User, user_id)
        if not user:
